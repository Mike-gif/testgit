            ...
            DataColumn dc = new DataColumn();
             ...
             dt.Columns.Add(dc);
                  ...
                  DataRow dr = dt.NewRow();
                   ...
                   dr[1] = ResolveUrl("~/images/QQ截图20191122180240.png");
                dt.Rows.Add(dr);
...
/*
            String sql27 = "SELECT * FROM [ZhouJiHua] where z_time='" + z_time.Text + "' and z_number='" + Session["number"] + "'";
            DataTable dt_allgongzuori1 = SqlHelper.ExecuteDataTable(sql27);
            int i_all = dt_allgongzuori1.Rows.Count;


            String sql28 = "SELECT * FROM [ShenQingShenBao] where s_time='" + z_time.Text + "' and s_number='" + Session["number"] + "' and s_leibie='周计划'";
            // System.Diagnostics.Debug.Assert(false, sql19);//打印调试信息
            DataTable dt_allgongzuori2 = SqlHelper.ExecuteDataTable(sql28);
            int j_all = dt_allgongzuori2.Rows.Count;


            if (i_all > 0 && j_all > 0)

                Response.Redirect("ShenQingShenBao2.aspx");
*/
 ...
 */
    ...
    protected void Calendar1_SelectionChanged(object sender, EventArgs e)
       {
           ...
           /* 
             ...
             z_time.Text += " " + getweek(newDate.DayOfWeek.ToString());
          */
          //z_time.Text= this.Calendar1.SelectedDate.DayOfWeek.ToString().Trim(); //得到选择的日期是周几
           // z_time.Text = Calendar1.SelectedDate.ToString("yyyy-MM-dd");//选择的日期是几号 yyyy-MM-dd
          //z_time.Text = DateTime.Now.ToString("yyyy-MM-dd"); 获取当前时间是几号
          // z_time.Text = DateTime.Now.DayOfWeek.ToString().Trim(); //得到当前时间是周几
        ...
        //推测出本周的所有日期
         ...
         /*  int num = Calendar1.SelectedDates.Count;
           ...
           {
               Label2.Text = Calendar1.SelectedDates[i].ToShortDateString();
           }
          */
       }
   ...
   public void GridViewDataBind()
     ...
     protected void gvShow_RowCommand(object sender, GridViewCommandEventArgs e)
     {
         ...
         string opername = e.CommandName;                //CommandName和CommandArgument来源于LINKBUTTON的参数传递，CommandName传递命令，CommandArgument传递参数,结合HTML,CommandArgument传递值为id值。
        ...
        //             System.Diagnostics.Debug.Assert(false, sql10);//打印调试信息
             ...
             if (operleibie == "周计划") //查询结果如果等于周计划
             {
                 ...
                 if (result_sheng > 0 && result_zhou > 0)
                     Response.Redirect("ShenQingShenBao2.aspx");
                 else if (result_sheng > 0 && result_zhou == 0)
                 {
                     while (result_zhou == 0)
                     {
                         string sql21 = "delete from [ZhouJiHua] where id=@id";
                         SqlParameter[] sps21 = new SqlParameter[] { 
                              new SqlParameter("@id", opervalue),
                                };
                         result_zhou = SqlHelper.ExecuteNonQuery(sql21, sps21);
                     }
                 }
                 else if (result_sheng == 0 && result_zhou > 0)
                 {
                     ...
                     {
                         string sql22 = "delete from [ShenQingShenBao] where id=@id";
                         SqlParameter[] sps22 = new SqlParameter[] { 
                     
                                 new SqlParameter("@id", opervalue),
                                 };
                         result_sheng = SqlHelper.ExecuteNonQuery(sql22, sps22);
                     }
                 }
             }
             ...
             else if (operleibie == "日计划")
             {
                 ...
                 string sql23 = "delete from [ShenQingShenBao] where id=@id";
                 SqlParameter[] sps23 = new SqlParameter[] { 
                     
                          new SqlParameter("@id", opervalue),
                           };
                 int result_sheng = SqlHelper.ExecuteNonQuery(sql23, sps23);

                 if (result_sheng > 0 && result_ri > 0)
                     Response.Redirect("ShenQingShenBao2.aspx");
                 // Response.Redirect("gvShow");
                 else if (result_sheng > 0 && result_ri == 0)
                 {
                     while (result_ri == 0)
                     {
                         string sql24 = "delete from [RiJiHua] where id=@id";
                         SqlParameter[] sps24 = new SqlParameter[] { 
                              new SqlParameter("@id", opervalue),
                                };
                         result_ri = SqlHelper.ExecuteNonQuery(sql24, sps24);
                     }
                 ...
                 else if (result_sheng == 0 && result_ri > 0)
                 {
                     while (result_sheng == 0)
                     {
                         string sql25 = "delete from [ShenQingShenBao] where id=@id";
                         SqlParameter[] sps25 = new SqlParameter[] { 
                     
                                new SqlParameter("@id", opervalue),
                                 };
                         result_sheng = SqlHelper.ExecuteNonQuery(sql25, sps25);
                     }
                 }
             ...
             else if (operleibie == "请假")
             {
                 ...
                 string sql26 = "delete from [ShenQingShenBao] where id=@id";
                 SqlParameter[] sps26 = new SqlParameter[] { 
                     
                      new SqlParameter("@id", opervalue),
                        };
                 int result_sheng = SqlHelper.ExecuteNonQuery(sql26, sps26);

                 if (result_sheng > 0 && result_qingjia > 0)
                     Response.Redirect("ShenQingShenBao2.aspx");
                 else if (result_sheng > 0 && result_qingjia == 0)
                 {
                     while (result_qingjia == 0)
                     {
                         string sql24 = "delete from [qingjia] where id=@id";
                         SqlParameter[] sps24 = new SqlParameter[] { 
                                new SqlParameter("@id", opervalue),
                                  };
                         result_qingjia = SqlHelper.ExecuteNonQuery(sql24, sps24);
                     }
                 }
                 else if (result_sheng == 0 && result_qingjia > 0)
                 {
    ...
    */
    ...
    protected void Btnri_Click(object sender, EventArgs e)  //日计划
    {
        ...
        String sql20 = "SELECT * FROM [ShenQingShenBao] where s_time='" + r_time.Text + "' and s_number='" + Session["number"] + "' and s_leibie='日计划'";
...
*/
            ...
            sps8[6] = new SqlParameter("@q_statue", "0");
